...
# Replace the existing EmailService class with the improved one
        ...
        try:
            if df is not None and len(df) > 0:
                with get_db_connection() as conn:
                    ...
                    thresholds = {setting['setting_name']: setting['threshold_value'] for setting in settings}
                    ...
                    # Detect anomalies
                    anomalies = []
                    anomalies.extend(anomaly_detector.detect_peak_power_anomalies(recent_data, thresholds))
                    anomalies.extend(anomaly_detector.detect_energy_spike_anomalies(recent_data, thresholds))
                    anomalies.extend(anomaly_detector.detect_anomalies(recent_data))
...
def create_alert_setting():
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            cursor.execute("SELECT id FROM alert_settings WHERE setting_name = ?", (data['setting_name'],))
            ...
            if existing:
                ...
                cursor.execute("""
                    INSERT INTO alert_settings (setting_name, threshold_value, threshold_type, is_enabled)
                    VALUES (?, ?, ?, ?)
                """, (
                    data['setting_name'],
                    data['threshold_value'],
...
def test_alert():
    ...
    try:
        ...
        # Send test email
        result = email_service.send_alert_email(test_alert_data, [test_email])
