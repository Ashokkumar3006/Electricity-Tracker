...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        # Alert settings table (updated to support device-specific thresholds)
        cursor.execute('''
    ...
    def add_alert_setting(self, setting_name, threshold_value, threshold_type, device_name=None, is_enabled=True):
        ...
        cursor.execute('''
            ...
            (setting_name, device_name, threshold_value, threshold_type, is_enabled, updated_at)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (setting_name, device_name, threshold_value, threshold_type, is_enabled, datetime.now()))
    ...
    def get_device_thresholds(self, device_name):
        ...
        cursor.execute('''
            ...
            FROM alert_settings 
            WHERE device_name = ? AND is_enabled = 1
        ''', (device_name,))
        ...
        for setting in settings:
            thresholds[setting[0]] = {
                'value': setting[1],
                'type': setting[2],
                'enabled': bool(setting[3])
            }
        
        return thresholds
    
    def get_all_device_thresholds(self):
        """Get all device-specific thresholds organized by device"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        device_thresholds = {}
        for setting in settings:
            device_name = setting[0]
            if device_name not in device_thresholds:
                device_thresholds[device_name] = {}
            
            device_thresholds[device_name][setting[1]] = {
                'value': setting[2],
                'type': setting[3],
                'enabled': bool(setting[4])
            }
